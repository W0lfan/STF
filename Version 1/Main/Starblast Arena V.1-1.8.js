...
var team1_name = "Orgono", team2_name = "Volgauf";
...
var internals_init = function() {if (game.custom.internals_init) {return;}const modding = game.modding;const internals = Object.values(modding).find(val => val && typeof val.shipDisconnected === "function");if (!internals) {modding.terminal.error("Error: modding internals object not found");return;}if (!internals.shipDisconnected.old) {function shipDisconnected({id} = {}) {if (modding.context.event && id) {var ship = game.findShip(id);if (ship) {modding.context.event({ name: "ship_disconnected", ship }, game);}}return shipDisconnected.old.apply(this, arguments);}shipDisconnected.old = internals.shipDisconnected;internals.shipDisconnected = shipDisconnected;}game.custom.internals_init = true;};
      ...
      for (let a = 0 ; a < game.ships.length ; a++) {
        if (game.ships[a].team === winner) {
          game.ships[a].gameover({
            ...
            " " : " ",
            "Orgono's scores (ship kills):" : `${Team1_ShipsKills}`,
            "Orgono's scores (alien kills):" : `${Team1_AliensKills}`,
            "Orgono's total score:" : team1_score,
            "Volgauf's scores (ship kills):" : `${Team2_ShipsKills}`,
            "Volgauf's scores (alien kills):" : `${Team2_AliensKills}`,
            "Volgauf's total score:" : team2_score,
          });
        } else {
          game.ships[a].gameover({
            "Thanks for playing Starblast Arena!":"Mod made by Wolfan",
            "Congratulations" : "You won!",
            "Your team" : game.ships[a].custom.team,
            "Deaths" : game.ships[a].custom.deaths,
            "Players kills" : game.ships[a].custom.frags,
...
var scoreboardACT = function(game) {
  if (team2_numbersP < 10) {
    scoreboard.components[16].value = `0${team2_numbersP}`;
  } else {
    scoreboard.components[16].value = `${team2_numbersP}`;
  }
  if (team1_numbersP < 10) {
    scoreboard.components[17].value = `0${team1_numbersP}`;
  } else {
    scoreboard.components[17].value = `${team1_numbersP}`;
  }
  if (team2_score < 10) {
...
var scoreboard = {
  ...
  components: [
    ...
    { type: "text",position:[30,-5,35,20],value: "Orgono",color:"#202223"},
    { type: "box",position:[0,50,100,10],stroke:"#4A83D6",fill:"#4A83D6",width:15},
    { type: "text",position:[30,45,35,20],value: "Volgauf" ,color:"#202223"},
    { type: "text",position:[0,49,22.5,12],value: " " ,color:"#202223"},
    { type: "text",position:[0,0,22.5,12],value: " " ,color:"#202223"},
    { type: "text",position:[75,49,22.5,12],value: " " ,color:"#202223"},
    { type: "text",position:[75,0,25,12],value: " " ,color:"#202223"},
...
var setPannels = function(ship) {
    if (ship.custom.pannelType === 1){
      ...
      ship6.components[1].value = "Penery";
...
var scores1 = {
  ...
  visible: true,
  components: [
    { type: "text",position:[12,0,30,40],value: " ",color:"#6DD56E"},
    { type: "text",position:[12,25,30,40],value: " ",color:"#6DD56E"},
    { type: "text",position:[49.5,0,30,40],value: " ",color:"#4A83D6"},
    { type: "text",position:[49.5,25,30,40],value: " ",color:"#4A83D6"},
