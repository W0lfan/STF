...
if (ship.type < Math.max(...FightingTierSix) ) {
  ship.set({type: ship.type + 1, shield:1000,generator:1000});
} else if (ship.type === Math.max(...FightingTierSix)) {
  ship.set({type:Math.min(...FightingTierSix),shield:1000,generator:1000});
};
  ...
  switch (event.name) {
    case "ui_component_clicked":
      if (component === "nextShip") {
        if (ship.type < Math.max(...FightingTierSix) ) {{
          ship.set({type: ship.type + 1,generator:0});
        }} else if (ship.type == Math.max(...FightingTierSix) ) {
          ship.set({type:Math.min(...FightingTierSix) });
        }
      }
